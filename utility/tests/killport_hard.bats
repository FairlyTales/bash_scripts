...
@test "terminates process on specified port with SIGKILL" {
    # Create a mock process on port 3000
    create_mock_process "3000" "12345"
    
    run "$UTILITY_SCRIPTS_PATH/killport_hard.sh" 3000
    ...
    # Verify kill was called with SIGKILL (-9)
...
@test "reports no process when port is empty" {
    ...
    run "$UTILITY_SCRIPTS_PATH/killport_hard.sh" 8080
...
@test "handles multiple processes on same port" {
    # Create multiple PIDs for the same port
    ...
    # Should attempt to kill the first PID found with -9
    assert_mock_called_with "$TEST_TEMP_DIR/kill_calls.log" "kill -9 12345"
}
...
@test "works with port 3001" {
    ...
    run "$UTILITY_SCRIPTS_PATH/killport_hard.sh" 3001
...
@test "handles invalid port argument gracefully" {
    ...
    run "$UTILITY_SCRIPTS_PATH/killport_hard.sh" "invalid"
...
@test "requires port argument" {
    run "$UTILITY_SCRIPTS_PATH/killport_hard.sh"
...
@test "uses SIGKILL signal for immediate termination" {
    create_mock_process "4000" "44444"
    
    run "$UTILITY_SCRIPTS_PATH/killport_hard.sh" 4000
    
    assert_success
    
    # Verify it uses -9 (SIGKILL) not -15 (SIGTERM)
    assert_mock_called_with "$TEST_TEMP_DIR/kill_calls.log" "kill -9 44444"
    
    # Ensure it's not using SIGTERM
    if grep -q "kill -15" "$TEST_TEMP_DIR/kill_calls.log" 2>/dev/null; then
        fail "Script should use SIGKILL (-9), not SIGTERM (-15)"
    fi
}

@test "difference from killport.sh - uses SIGKILL instead of SIGTERM" {
    create_mock_process "5000" "55555"
    
    run "$UTILITY_SCRIPTS_PATH/killport_hard.sh" 5000
    ...
    # Verify the output message is the same as killport.sh
    assert_output --partial "Process on port 5000 terminated"
}