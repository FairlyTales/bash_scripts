...
@test "successfully launches Cursor with current directory" {
    cd "$TEST_TEMP_DIR"
    
    run "$IDE_SCRIPTS_PATH/launch_cursor_in_pwd.sh"
...
@test "works with special characters in directory path" {
    ...
    assert_success
    assert_output --partial "Launching Cursor in $special_dir"
    assert_ide_called_with "cursor $special_dir"
}

@test "handles cursor command failure" {
...
@test "passes quoted path to cursor command" {
    cd "$TEST_TEMP_DIR"
    
    run "$IDE_SCRIPTS_PATH/launch_cursor_in_pwd.sh"
    
    assert_success
    # Verify the cursor command receives the path in quotes
    assert_ide_called_with "cursor $TEST_TEMP_DIR"
}